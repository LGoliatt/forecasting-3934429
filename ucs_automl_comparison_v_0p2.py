...
def permutation_feature_importance(
    model_name, model, X_test, y_test, scoring_func=mean_squared_error, n_repeats=100
):
    """
    ...
    if model_name == "AutoGluon":
        ...
        baseline_score = scoring_func(
            y_test, model.predict(test_df).as_data_frame().values.ravel()
        )
    else:
    ...
    for dataset in datasets:
        ...
        for tk, tn in enumerate(dataset["target_names"]):
            ...
            pipeline_optimizer = TPOTRegressor(
                ...
                verbosity=False,
                n_jobs=2,  # Multithreading added by ROMULO MURUCCI
                max_time_mins=2,  # in minutes  # Added by ROMULO MURUCCI
            )
            ...
            for auto in [
                ...
                if auto == "FLAML":
                    ...
                    automl = TabularPredictor(label=target).fit(
                        ...
                        verbosity=False,
                        time_limit=time_budget,  # Added by ROMULO MURUCCI
                        # num_cpus=2,  # The model already evaluates multithreading automatically
                    )
                    ...
                    automl = H2OAutoML(
                        max_runtime_secs=time_budget, seed=seed, sort_metric="rmse"
                    )
                    automl.train(
